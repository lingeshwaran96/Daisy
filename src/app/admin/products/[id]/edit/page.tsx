...
import { Upload, X, Plus, Save, Link as LinkIcon, ImagePlus, Loader2, ArrowLeft } from 'lucide-react';
...
export default function EditProductPage() {
    ...
    async function load() {
      ...
      if (product) {
        ...
        if (Array.isArray(p.variants)) {
          setVariants((p.variants as any[]).map((v: any) => ({
            name: v.name || '',
        ...
        if (p.specifications && typeof p.specifications === 'object') {
          setSpecs(Object.entries(p.specifications as Record<string, string>).map(([key, value]) => ({ key, value })));
        }
      } else {
  ...
  const handleAddUrl = () => {
    if (!urlInput.trim() || !urlInput.startsWith('http')) { toast.error('Enter a valid URL'); return; }
  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl">
      <div className="mb-8 flex items-center gap-4">
        <button onClick={() => router.push('/admin/products')} className="p-2 text-daisy-400 hover:text-daisy-900 transition-colors">
          <ArrowLeft size={20} />
        </button>
        <div>
          <h1 className="font-heading text-4xl text-daisy-900 font-light">Edit Product</h1>
          <p className="font-body text-sm text-daisy-500 mt-1">Update the product details below</p>
        </div>
      </div>
        ...
        <section className="bg-white border border-nude-200 p-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" value={form.material} onChange={set('material')} placeholder="e.g. 925 Sterling Silver"
                className="w-full border border-nude-200 px-4 py-3 font-body text-sm text-daisy-900 outline-none focus:border-daisy-400 transition-colors" />
            </div>
            <div className="md:col-span-2">
              <label className="block font-body text-xs tracking-widest uppercase text-daisy-500 mb-2">Full Description</label>
        ...
        <section className="bg-white border border-nude-200 p-6">
          ...
          <div className="flex gap-2 mb-6">
            <button type="button" onClick={() => setImageMode('upload')}
              className={`flex items-center gap-2 px-4 py-2.5 font-body text-sm border transition-all duration-200 ${imageMode === 'upload' ? 'bg-daisy-900 text-cream border-daisy-900' : 'border-nude-200 text-daisy-600 hover:border-daisy-400'}`}>
              <ImagePlus size={16} /> Upload Image
            </button>
            <button type="button" onClick={() => setImageMode('url')}
              className={`flex items-center gap-2 px-4 py-2.5 font-body text-sm border transition-all duration-200 ${imageMode === 'url' ? 'bg-daisy-900 text-cream border-daisy-900' : 'border-nude-200 text-daisy-600 hover:border-daisy-400'}`}>
            ...
            <div className="mb-6">
              ...
              <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                ...
                {uploading
                  ...
                  : <><Upload size={32} className="text-daisy-300" /><span className="font-body text-sm text-daisy-600">Click to upload images</span><span className="font-body text-[11px] text-daisy-400">PNG, JPG, WEBP up to 5MB each</span></>
                }
              </button>
            </div>
          )}
          ...
          {images.length > 0 && (
            <div>
              <p className="font-body text-xs tracking-widest uppercase text-daisy-500 mb-3">
                {images.length} Image{images.length !== 1 ? 's' : ''} — drag to reorder (coming soon)
              </p>
              <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                {images.map((url, i) => (
                  <div key={i} className="relative group aspect-square bg-nude-100 border border-nude-200 overflow-hidden">
                    ...
                    {i === 0 && (
                      <span className="absolute top-1 left-1 bg-daisy-700 text-cream font-body text-[9px] px-1.5 py-0.5 tracking-widest uppercase">Main</span>
                    )}
            ...
            {variants.map((v, i) => (
              <div key={i} className="flex gap-3">
                ...
                <button type="button" onClick={() => setVariants(variants.filter((_, j) => j !== i))}
                  className="p-3 text-daisy-400 hover:text-red-500 border border-nude-200 transition-colors"><X size={16} /></button>
              </div>
            ))}
